...
namespace AuftEftMain.Helpers
{
    class ConfigHelper
    {
        public static string SelectedConfig = "CFG1";
        //rivate static string ConfigPath = "C:\\Battlestate Games\\Config\\";
        private static readonly string ConfigPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments), "Auft\\");
        private static string GetConfigPath(string name = "CFG1") { return ConfigPath + name + ".Auft"; }
        ...
        public static void SaveConfig(string name = "CFG1", bool setconfig = false)
        {
            string path = GetConfigPath(name);
            string json = EncryptStatic(JsonConvert.SerializeObject(Globals.Config, Formatting.Indented));
        ...
        public static void LoadConfig(string name = "CFG1")
        {
            if (File.Exists(GetConfigPath(name)))
            {
                string json = File.ReadAllText(GetConfigPath(name));
                BaseConfig s = JsonConvert.DeserializeObject<BaseConfig>(DecryptStatic(json));
                Globals.Config = s;
                SelectedConfig = name;
        ...
        public static List<string> GetConfigs(bool Extensions = false)
        {
            ...
            FileInfo[] Files = d.GetFiles("*.Auft");
